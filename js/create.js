...
function addCharacterCard(data) {
  ...
  if (data) {
    ...
    card.querySelector(".char-platonic").checked = !!data.platonic;
  }
...
// feeling stats when the reader just tweaks their appearance/backstory
// text rather than resetting them to 0.
//
// Feeling stats are 3 center-oriented axes, each -100..100, starting at 0:
...
function collectCharacters(existingCharacters = []) {
  ...
  return Array.from(document.querySelectorAll(".character-card")).map((card) => {
    ...
    return {
      ...
      platonic: card.querySelector(".char-platonic").checked,
      stats: existing && existing.stats ? existing.stats : { love: 0, trust: 0, loyalty: 0 }
    };
  }).filter((c) => c.name);
}
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const bookData = {
      ...
      // AI-generated epithet that drifts as the story goes based on how
      // the reader plays (e.g. "a hermit", "a reckless daredevil").
      // Starts unset; the AI fills it in once it has enough to go on.
      publicView: "",
