...
function applySideEffects(variant) {
  ...
  if (result.statChanges) {
    for (const [charName, changes] of Object.entries(result.statChanges)) {
      ...
      character.stats = character.stats || { love: 0, trust: 0, fear: 0, jealousy: 0, loyalty: 0 };
      for (const [statName, delta] of Object.entries(changes)) {
        const current = character.stats[statName] ?? 0;
        character.stats[statName] = Math.max(0, Math.min(100, current + delta));
      }
    }
  }
...

{
  ...
  ${optionsSchema},
  "statChanges": { "characterName": { "love": number, "trust": number, "fear": number, "jealousy": number, "loyalty": number } },
...
- Only include characters/stats that actually change in "statChanges". Stats range 0-100.
...
Characters:
${characters.map(c => `- ${c.name}: ${c.appearance || ""}. Backstory: ${c.backstory || ""}. Behavior: ${c.behavior || ""}. Starting relationship to protagonist: ${c.startingRelationship || "Stranger"}. Never-forget trait: ${c.neverForgetTrait || "none"}. Current feelings toward protagonist: love ${c.stats?.love ?? 0}%, trust ${c.stats?.trust ?? 0}%, fear ${c.stats?.fear ?? 0}%, jealousy ${c.stats?.jealousy ?? 0}%, loyalty ${c.stats?.loyalty ?? 0}%.`).join("\n")}
